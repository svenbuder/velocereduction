...
def raw_fits_path(paths, night, run, ccd):
    """
    Return the expected raw Veloce FITS filename.
    """
    ...
    return (
        ...
        / night
        / f'ccd_{ccd}'
        / f'{night[-2:]}{month}{ccd}{int(run):04d}.fits'
    )
...
def _robust_location_and_sigma(values):
...
def preprocess_image(filename, ccd):
    """
    Read and apply common detector preprocessing to one Veloce image.

    Dark subtraction and observation-specific corrections are deliberately
    not performed here.
    """
    ...
    image, overscan_median, overscan_rms, readout_mode = (
        subtract_overscan(
            full_image,
            metadata,
        )
    ...
    detector_info = {
        ...
        'overscan_median': overscan_median,
        'overscan_rms': overscan_rms,
    }

    return image, metadata, detector_info
...
def robust_sigma(values):
    """Robust estimate of 1-sigma scatter from the 16th/84th percentiles."""
...
def shifted_coefficients(coeffs, dx=0.0, dy=0.0):
    """
    Shift polynomial y(x) by detector displacement dx, dy.

    If the current detector position is:

        y_current(x) = y_reference(x - dx) + dy

    this returns the coefficients describing y_current(x).

    Coefficients follow numpy.polynomial.Polynomial convention:
        c0 + c1*x + c2*x**2 + ...
    """
